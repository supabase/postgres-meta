...
test('typegen: typescript excludes generated stored columns from Insert and Update types', async () => {
  await app.inject({
    method: 'POST',
    path: '/query',
    payload: {
      query: `
        DROP TABLE IF EXISTS test_generated_col CASCADE;
        CREATE TABLE test_generated_col (
  ...
  try {
    ...
    // The generated stored column should appear in Row
    expect(body).toContain('"total": number')

    // The generated stored column should be excluded from Insert (?: never)
    const insertBlockMatch = body.match(/Insert:\s*\{([^}]*)\}/s)
    expect(insertBlockMatch).not.toBeNull()
    if (insertBlockMatch) {
      const insertBlock = insertBlockMatch[1]
      expect(insertBlock).toContain('"total"?: never')
      expect(insertBlock).not.toMatch(/"total"\?:\s*number/)
    }

    // The generated stored column should be excluded from Update (?: never)
    const updateBlockMatch = body.match(/Update:\s*\{([^}]*)\}/s)
    expect(updateBlockMatch).not.toBeNull()
    if (updateBlockMatch) {
      const updateBlock = updateBlockMatch[1]
      expect(updateBlock).toContain('"total"?: never')
      expect(updateBlock).not.toMatch(/"total"\?:\s*number/)
    }
  } finally {
    await app.inject({
      method: 'POST',
      path: '/query',
      payload: {
        query: `DROP TABLE IF EXISTS test_generated_col CASCADE;`,
      },
    })
  }
})
