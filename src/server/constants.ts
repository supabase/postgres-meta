...
const PG_META_DB_SSL_ROOT_CERT = process.env.PG_META_DB_SSL_ROOT_CERT || ''
...
export const PG_CONNECTION =
  ...
  `postgres://${PG_META_DB_USER}:${encodeURIComponent(
    PG_META_DB_PASSWORD
  )}@${PG_META_DB_HOST}:${PG_META_DB_PORT}/${PG_META_DB_NAME}` +
  `?sslmode=${PG_META_DB_SSL_MODE}&sslrootcert=${PG_META_DB_SSL_ROOT_CERT}`
